  ...
  return (
    <div className="reveal max-w-[1280px] mx-auto px-[22px] py-[clamp(20px,4vw,32px)] space-y-6">
      ...
      <div>
        ...
        <p className="text-[13px] text-mute mt-1.5 leading-relaxed">维护机构层级与部门用户角色配置。</p>
      </div>

      {/* Organizations panel — unified list with hairline dividers */}
      <section className="bg-white rounded-[12px] p-6" style={{ boxShadow: 'var(--sh-panel)' }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[15px] font-bold tracking-[-0.01em] text-ink">机构列表</h2>
          <button onClick={createDepartment}
            className="h-11 px-5 bg-ink hover:bg-inkhover text-white text-[13px] font-semibold rounded-md transition-colors">
            新增部门
          </button>
        </div>
        <div>
          {targets.map(c => (
            <div key={c.id} className="apple-row py-3.5 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5 text-[13px] min-w-0">
                ...
                <span className="text-[12px] text-mute shrink-0 tabular-nums">· {c.level}</span>
              </div>
              <button
                disabled={c.status !== 'active'}
                onClick={async () => { if (await confirmDialog(`确认停用 ${c.name}？`)) { await api.disableCompany(c.id); await load(); } }}
                className={`text-[12px] font-semibold px-3 h-8 rounded-full transition-colors ${c.status === 'active' ? 'text-[#9F2F2D] hover:bg-[#FDEBEC]' : 'text-faint cursor-not-allowed'}`}>
      ...
      <section className="bg-white rounded-[12px] p-6" style={{ boxShadow: 'var(--sh-panel)' }}>
        <h2 className="text-[15px] font-bold tracking-[-0.01em] text-ink mb-4">用户配置</h2>
        <div>
          {users.map(u => (
            <div key={u.id} className="apple-row py-3.5 grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
              <span className="text-[13px] font-semibold text-ink truncate">{u.display_name}</span>
              <select value={u.company_id} onChange={e => updateUser(u, Number(e.target.value), u.role)}
                className="h-11 px-4 bg-canvas rounded-[12px] text-[13px] text-ink focus:outline-none focus:ring-1 focus:ring-ink focus:bg-white">
