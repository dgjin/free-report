  ...
  return (
    <div className="max-w-[1080px] mx-auto px-[22px] py-[clamp(20px,4vw,32px)] space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="px-3.5 py-1.5 bg-[rgba(0,113,227,0.08)] text-[#0071e3] rounded-full text-xs font-medium shrink-0 tabular-nums">
      ...
      {loading ? (
        <div className="py-16 text-center text-sm text-[#86868b]">正在获取审批列表...</div>
      ) : pendingTasks.length === 0 ? (
        <div className="bg-white rounded-[22px] py-16 text-center" style={{ boxShadow: 'var(--sh-card)' }}>
          <CheckCircle2 className="w-10 h-10 text-[#d2d2d7] mx-auto mb-3" />
          <div className="text-sm font-medium text-[#1d1d1f]">暂无待您审批的报表任务</div>
          <p className="text-xs text-[#86868b] mt-1">所有的报表流转已被高效处理完成</p>
        </div>
      ) : (
            ...
            >
              <div className="space-y-2 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2.5 py-1 bg-[#f5f5f7] text-[#6e6e73] text-[11px] font-medium rounded-full">
                    {task.period_label}
                  </span>
                  <h3 className="text-base font-semibold text-[#1d1d1f] tracking-[-0.01em]">{task.assignment_title}</h3>
                </div>

                <div className="flex items-center gap-4 text-xs text-[#86868b] flex-wrap gap-y-1">
                  <div className="flex items-center gap-1.5">
                    <Building2 className="w-3.5 h-3.5 text-[#aeaeb2]" />
                    <span>填报公司: {task.company_name}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <UserCheck className="w-3.5 h-3.5 text-[#aeaeb2]" />
                  ...
                  <span className="font-medium text-[#6e6e73] tabular-nums">v{task.version}</span>
                </div>
              </div>

              <button
                onClick={() => openReviewModal(task)}
                className="h-10 px-5 bg-[#0071e3] hover:bg-[#0066cc] text-white font-medium text-sm rounded-full transition-colors flex items-center justify-center gap-1.5 shrink-0"
              >
      ...
      {selectedTask && (
        ...
        >
          <div
            className="bg-white rounded-[22px] max-w-3xl w-full p-6 space-y-5 animate-in fade-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            {detailLoading || !submissionDetail ? (
              <div className="py-12 text-center text-sm text-[#86868b]">正在加载数据明细...</div>
            ) : (
              <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                    {submissionDetail.summary.map((s) => (
                      <div key={s.field_id} className="bg-white p-3 rounded-[12px] border border-[rgba(0,0,0,0.06)]">
                        ...
                        <div className="text-sm font-semibold text-[#1d1d1f] mt-0.5 tabular-nums">
                          {s.value || '(未填写)'}
                        </div>
                      </div>
                    ))}
                ...
                <div className="space-y-3 pt-1">
                  <label className="block text-xs font-medium text-[#424245]">
                    处理意见 (驳回时请必须填写具体原因)
                  </label>
                  <textarea
                    rows={2}
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2 border-t border-[rgba(0,0,0,0.07)]">
                    <button
                      type="button"
                      disabled={processing}
                      onClick={() => handleAction('rejected')}
                      className="h-10 px-5 bg-[rgba(255,107,0,0.1)] hover:bg-[rgba(255,107,0,0.18)] text-[#ff6b00] font-medium text-sm rounded-full transition-colors flex items-center gap-1.5 disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      className="h-10 px-5 bg-[#0071e3] hover:bg-[#0066cc] text-white font-medium text-sm rounded-full transition-colors flex items-center gap-1.5 disabled:opacity-50"
                    >
      ...
      {toast && (
        <div
          className={`fixed top-6 right-6 z-[60] px-5 py-3 rounded-full text-sm font-medium animate-in fade-in slide-in-from-top-2 duration-200 text-white ${
            toast.type === 'error' ? 'bg-[#ff6b00]' : 'bg-[#1d1d1f]'
          }`}
        >
          {toast.message}
        </div>
